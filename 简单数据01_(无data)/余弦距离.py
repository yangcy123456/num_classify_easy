...
# ====================== 余弦距离分类 ======================
def predict_cosine(sample, means):
    # 👇 原来：欧式距离（效果差）
    ...
    for i in range(10):
        # 余弦距离 = 1 - 余弦相似度（值越小越像）
        cos_sim = np.dot(sample.flatten(), means[i].flatten()) / (
            np.linalg.norm(sample.flatten()) * np.linalg.norm(means[i].flatten()) + 1e-8
        )
...
if __name__ == "__main__":
    ...
    # 3. 预测
    y_pred = [predict_cosine(s, class_means) for s in X]
